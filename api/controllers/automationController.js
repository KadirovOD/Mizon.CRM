...
exports.runTrigger = async (db, triggerType, lead, extra = {}) => {
  ...
  try {
    ...
    for (const rule of rules) {
      ...
      const msg = fillTemplate(rule.template_msg || rule.action_message || '', lead, stageName);
...
exports.createRule = async (req, res) => {
  if (!req.db) return res.status(503).json({ error: 'DB not configured' });
  const cid = req.user?.companyId;
  const { name, trigger_type, template_id, stage_filter } = req.body;
  if (!name || !trigger_type || !template_id) return res.status(400).json({ error: 'Nom, trigger va shablon majburiy' });
  try {
    const r = await req.db.query(
      'INSERT INTO automation_rules (company_id, name, trigger_type, template_id, stage_filter) VALUES ($1,$2,$3,$4,$5) RETURNING *',
      [cid, name, trigger_type, template_id, stage_filter||null]
    );
    res.status(201).json(r.rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
};
...
exports.updateRule = async (req, res) => {
  if (!req.db) return res.status(503).json({ error: 'DB not configured' });
  const cid = req.user?.companyId;
  const { name, trigger_type, template_id, stage_filter, is_active } = req.body;
  try {
    const r = await req.db.query(
      ...
      [name||null, trigger_type||null, template_id||null, stage_filter||null, is_active??null, req.params.id, cid]
    );
    if (!r.rows.length) return res.status(404).json({ error: 'Topilmadi' });
    res.json(r.rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
