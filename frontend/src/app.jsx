          ...
          {activeModal && typeof activeModal === 'object' && (() => {
            ...
            const SheetsFlow = () => {
              ...
              const script = [
                ...
                `const HEADER_MAP = {`,
                `  'full_name':    'name_meta',`,
                `  'first_name':   'name_meta',`,
                `  'last_name':    'name_last',`,
                `  'phone_number': 'phone_meta',`,
                `  'phone':        'phone_meta',`,
                `  'email':        'email_meta',`,
                `  'city':         'region_meta',`,
                `  'country':      'region_meta',`,
                ...
                `};`,
                ``,
                `// MUTLAQO E'TIBORSIZ ustunlar (ID, vaqt, reklama metadata)`,
                `// Bu ustunlar UZ_PATTERNS ham, HEADER_MAP ham tegmaydi`,
                `const SKIP_HEADERS = [`,
                `  'id','lead_id','leadgen_id','created_time','created_date','updated_time',`,
                `  'ad_id','ad_name','adset_id','adset_name','campaign_id','campaign_name',`,
                `  'form_id','form_name','page_id','page_name','platform','is_organic',`,
                `  'partner_name','partner','source','status'`,
                `];`,
                ``,
                `// Foydalanuvchi qo'lda yozgan custom maydonlar (Meta nomlardan USTUN qo'yiladi)`,
                `// Masalan: "telefon_raqamingizni_yozing!", "ismingizni_yozing!" va h.k.`,
                `const UZ_PATTERNS = [`,
                `  { rx: /(\\bism\\b|ismi|ismingiz|familiya|f_i_o|fish)/i, field: 'name_uz' },`,
                `  { rx: /(telefon|raqam|\\btel\\b|nomer|\\bphone\\b)/i,    field: 'phone_uz' },`,
                `  { rx: /(manzil|shahar|hudud|viloyat|address)/i,         field: 'region_uz' },`,
                `  { rx: /(email|mail|pochta)/i,                            field: 'email_uz' },`,
                `  { rx: /(izoh|sharh|\\bcomment\\b|xabar|\\bnote\\b)/i,    field: 'note_uz' },`,
                `];`,
                ``,
                `// Qiymat haqiqiy telefon raqamga o'xshashligini tekshirish`,
                `function looksLikePhone(s) {`,
                `  if (!s) return false;`,
                `  var str = s.toString();`,
                `  // ISO timestamp emasligi`,
                `  if (/^\\d{4}-\\d{2}-\\d{2}[T ]/.test(str)) return false;`,
                `  var digits = str.replace(/\\D/g, '');`,
                `  return digits.length >= 7 && digits.length <= 15;`,
                `}`,
                `function cleanPhone(s) {`,
                `  return s.toString().trim().replace(/^p\\s*:/i, '').trim();`,
                `}`,
                `// Qiymat haqiqiy ismga o'xshashligini tekshirish`,
                `function looksLikeName(s) {`,
                `  if (!s) return false;`,
                `  var str = s.toString().trim();`,
                `  if (!str) return false;`,
                `  if (/^[a-z]+\\s*:\\s*\\d{6,}$/i.test(str)) return false; // Meta id (l:12345...)`,
                `  if (/^\\d{4}-\\d{2}-\\d{2}[T ]/.test(str)) return false; // timestamp`,
                `  if (/^\\d{8,}$/.test(str)) return false;                 // sof raqam`,
                `  return true;`,
                `}`,
                ``,
                `// ─── Asosiy funksiya ──────────────────────────────────────`,
                ...
                `    if (!hl) return;`,
                `    // 0) Noise ustunlarni tashlab yuborish (id, created_time, ad_name, ...)`,
                `    if (SKIP_HEADERS.indexOf(hl) >= 0) return;`,
                `    // 1) Aniq Meta nomlari`,
                `    var exact = HEADER_MAP[hl];`,
                `    if (exact && colIdx[exact] === undefined) colIdx[exact] = i;`,
                `    // 2) Custom (Uzbek/foydalanuvchi yozgan) maydonlar — bular ustunroq`,
                `    UZ_PATTERNS.forEach(function(p) {`,
                `      if (HEADER_MAP[hl]) return; // aniq Meta nomi bo'lsa custom emas`,
                `      if (p.rx.test(hl) && colIdx[p.field] === undefined) colIdx[p.field] = i;`,
                `    });`,
                `  });`,
                ...
                `    var row = data[i];`,
                ``,
                `    // ISM: avval foydalanuvchi yozgan custom maydon, keyin Meta full_name`,
                `    var nmUz  = colIdx['name_uz']   !== undefined ? String(row[colIdx['name_uz']]   || '').trim() : '';`,
                `    var nmMt  = colIdx['name_meta'] !== undefined ? String(row[colIdx['name_meta']] || '').trim() : '';`,
                `    var lastN = colIdx['name_last'] !== undefined ? String(row[colIdx['name_last']] || '').trim() : '';`,
                `    var fullName = '';`,
                `    if (looksLikeName(nmUz)) fullName = nmUz;`,
                `    else if (looksLikeName(nmMt) || looksLikeName(lastN)) fullName = [nmMt, lastN].filter(Boolean).join(' ').trim();`,
                ``,
                `    // TELEFON: bir nechta nomzodlar — birinchisi haqiqiy raqamga o'xshasa olamiz`,
                `    var phoneCandidates = [];`,
                `    if (colIdx['phone_uz']   !== undefined) phoneCandidates.push(cleanPhone(row[colIdx['phone_uz']]   || ''));`,
                `    if (colIdx['phone_meta'] !== undefined) phoneCandidates.push(cleanPhone(row[colIdx['phone_meta']] || ''));`,
                `    var rawPhone = '';`,
                `    for (var j = 0; j < phoneCandidates.length; j++) {`,
                `      if (looksLikePhone(phoneCandidates[j])) { rawPhone = phoneCandidates[j]; break; }`,
                `    }`,
                `    // Fallback: hamma ustunlardan haqiqiy telefon qidirish`,
                `    if (!rawPhone) {`,
                `      for (var k = 0; k < row.length; k++) {`,
                `        if (k === colIdx['phone_uz'] || k === colIdx['phone_meta']) continue;`,
                `        var cand = cleanPhone(row[k] || '');`,
                `        if (looksLikePhone(cand)) { rawPhone = cand; break; }`,
                `      }`,
                `    }`,
                ``,
                `    var email  = (colIdx['email_uz']  !== undefined ? row[colIdx['email_uz']]  : (colIdx['email_meta']  !== undefined ? row[colIdx['email_meta']]  : '')) || '';`,
                `    var region = (colIdx['region_uz'] !== undefined ? row[colIdx['region_uz']] : (colIdx['region_meta'] !== undefined ? row[colIdx['region_meta']] : '')) || '';`,
                `    var note   = (colIdx['note_uz']   !== undefined ? row[colIdx['note_uz']]   : (colIdx['note_meta']   !== undefined ? row[colIdx['note_meta']]   : '')) || '';`,
                ``,
                `    var payload = {`,
...
.then(d => {
  if (d.success) window.location.href = '/rahmat.html';
    ...
    // ===== V66: XODIM KO'RSATKICHLARI — ODDIY CHIZIQLI DIAGRAMMA =====
    // Jadval pastida har bir xodim uchun 5 ta ko'rsatkich bo'yicha oddiy
    // gorizontal chiziqli (bar) diagramma — kuchli/zaif tomonlarni ko'rsatadi.
    const PERF_METRICS = [
      { key:'convRate',      label:'YAKUN' },
      { key:'ansRate',       label:'ALOQA' },
      { key:'activityScore', label:'FAOLLIK' },
      { key:'taskRate',      label:'INTIZOM', fallback:0 },
      { key:'reliability',   label:'BARQAROR' },
    ];

    const perfScoreColor = (s) => s>=70 ? '#01a750' : s>=40 ? '#f59e0b' : '#ef4444';

    const EmployeeBarChart = ({ r, isTop }) => {
      const color = perfScoreColor(r.score);
      return (
        <div style={{background:'var(--bg-surface)', border:'1px solid var(--outline-variant)', borderRadius:'12px', padding:'14px 16px'}}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px'}}>
            <div style={{fontWeight:700, fontSize:13}}>
              {isTop && <span title="Yetakchi" style={{marginRight:'4px'}}>🏆</span>}
              {r.fullName}
            </div>
            <div style={{fontWeight:800, fontSize:15, color}}>{r.score}</div>
          </div>
          <div style={{display:'flex', flexDirection:'column', gap:'7px'}}>
            {PERF_METRICS.map(m => {
              const v = r[m.key] ?? m.fallback ?? 0;
              return (
                <div key={m.key} style={{display:'flex', alignItems:'center', gap:'8px'}}>
                  <div style={{width:'62px', fontSize:10.5, color:'var(--text-muted)', fontWeight:600}}>{m.label}</div>
                  <div style={{flex:1, background:'var(--outline-variant)', borderRadius:'5px', height:'8px', overflow:'hidden'}}>
                    <div style={{width:`${Math.max(2,v)}%`, height:'100%', background:perfScoreColor(v), borderRadius:'5px'}} />
                  </div>
                  <div style={{width:'26px', textAlign:'right', fontSize:10.5, fontWeight:700}}>{v}</div>
                </div>
              );
            })}
          </div>
    ...
    const PerformanceModule = ({ leads, getAuthHeaders, authUser, setSelectedLeadId }) => {
      ...
      return (
        <div style={{padding:'24px'}}>
          ...
          {sorted.length > 0 && (
            <>
              <h3 style={{fontSize:'15px', fontWeight:700, marginTop:'26px', marginBottom:'12px'}}>📊 Ko'rsatkichlar diagrammasi</h3>
              <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(260px, 1fr))', gap:'14px'}}>
                {sorted.map((r, i) => (
                  <EmployeeBarChart key={r.username} r={r} isTop={i===0 && r.score>0} />
                ))}
              </div>
            </>
          )}

          <p style={{fontSize:11, color:'var(--text-muted)', marginTop:'14px'}}>
            Umumiy ball = konversiya (30%) + qo'ng'iroqqa javob % (30%) + vazifa bajarilishi (40%, agar davr uchun vazifa bo'lmasa e'tiborga olinmaydi). Diagrammadagi ko'rsatkichlar: YAKUN (konversiya), ALOQA (qo'ng'iroqqa javob %), FAOLLIK (qo'ng'iroqlar hajmi, jamoadagi eng faolga nisbatan), INTIZOM (vazifa bajarilishi), BARQAROR (kechikishlar qanchalik kam).
          </p>
        </div>
      );
    };
    ...
    const ActivityLogPanel = ({ leads, columnsMap, companyUsers, setSelectedLeadId }) => {
      ...
      (leads || []).forEach(l => {
        ...
        sortedAsc.forEach(({ log, _origIdx }) => {
    ...
    const MarketingModule = () => {
      ...
      const _mktAuth  = _mktToken ? {'Content-Type':'application/json','Authorization':'Bearer '+_mktToken} : {'Content-Type':'application/json'};
      ...
      useEffect(() => {
        ...
        saveMktToServer({campaigns, tgChannels: JSON.parse(localStorage.getItem(TG_KEY)||'[]')});
      }, [campaigns]);
      // Birinchi mount'da serverdan yuklash
      useEffect(() => {
        if (!_mktToken) { _mktLoaded.current = true; return; }
        fetch('/api/company/settings', {headers:_mktAuth})
          .then(r => r.ok ? r.json() : null)
          .then(d => {
          ...
          .catch(()=>{})
          .finally(() => { _mktLoaded.current = true; });
      }, []);
      ...
      useEffect(() => {
        ...
        saveMktToServer({campaigns, tgChannels});
      }, [tgChannels]);
    ...
    const PublicLeadFormView = ({ companySlug, formPipeId }) => {
    ...
    const App = () => {
      ...
      if (isFormMode) return <PublicLeadFormView companySlug={companySlug} formPipeId={formPipeId} />;
      ...
      const _notifTypes = { sla_danger:'🔴', sla_warning:'🟡', new_lead:'🆕', stage_changed:'🔄', call_limit:'📵', call_logged:'📞', voip_incoming:'📲', ext_lead:'🌐', task_assigned:'📌', billing:'💳' };
      ...
      const kanbanDragScroll = React.useRef({ active:false, startX:0, scrollLeft:0 });
      ...
      const onKanbanMouseDown = (e) => {
        ...
        const el = kanbanBoardRef.current;
        if (!el) return;
        kanbanDragScroll.current = { active:true, startX: e.pageX - el.offsetLeft, scrollLeft: el.scrollLeft };
      ...
      const onKanbanMouseUp = () => { kanbanDragScroll.current.active = false; setKanbanGrabbing(false); };
      ...
      const importLeadsFromCsv = async (file) => {
        ...
        if (lines.length < 2) { alert("Fayl bo'sh yoki sarlavhasi yetishmaydi."); return; }
        ...
        const token = localStorage.getItem('mizon_token');
        const auth = token ? {'Content-Type':'application/json','Authorization':'Bearer '+token} : {'Content-Type':'application/json'};
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          try {
            const r = await fetch('/api/leads', { method:'POST', headers: auth, body: JSON.stringify(payload) });
      ...
      const handleDragEnd = () => { setDraggingLeadId(null); };
                      ...
                      {editingLead && role !== 'WATCHER' ? (
                        ...
                        <div style={{marginBottom:'18px'}}>
                          ...
                          {(() => {
                            ...
                            return Object.keys(cd).filter(k => !known.has(k) && cd[k] != null && cd[k] !== '').map(k => (
                              <div key={'auto_'+k} className="info-row">
                                <div className="info-row-icon"><span className="material-symbols-outlined" style={{fontSize:'15px'}}>auto_awesome</span></div>
                                <span className="info-label">{k.replace(/_/g,' ').replace(/^./, c=>c.toUpperCase())}</span>
                                <span className="info-value">{String(cd[k])}</span>
                              </div>
                            ));
                          })()}
                        </div>
              ...
              {activeTab === 'leads' && (
                <div style={{display:'flex', flexDirection:'column', height:'100%'}}>
                      ...
                      {role === 'CEO' && (
                        <>
                          <input type="file" ref={csvFileInputRef} accept=".csv,text/csv" style={{display:'none'}}
                            onChange={e => { const f = e.target.files?.[0]; if (f) importLeadsFromCsv(f); e.target.value=''; }} />
                          <button className="btn-outline" onClick={()=>csvFileInputRef.current?.click()} title="CSV faylidan lidlarni yuklash (ustunlar: ism, telefon, hudud, ...)">
                            <Ico n="upload" s={13}/> Import (CSV)
                          </button>
                        </>
                      )}
                  ...
                  <div className="kanban-board" ref={kanbanBoardRef}
                    ...
                    style={{cursor: kanbanGrabbing ? 'grabbing' : 'grab', userSelect: kanbanGrabbing ? 'none' : undefined}}>
                            ...
                            {colLeads.map(lead => {
                              ...
                              return (
                                <div key={lead.id} className={`k-card ${customClass}`}
                                  style={{borderLeftColor: customClass ? undefined : dotColor, position:'relative', outline: isSelected ? '2px solid var(--primary)' : undefined, background: isSelected ? 'var(--primary-soft)' : undefined}}
                  ...
                  {selectMode && selectedIds.size > 0 && (
                    <div style={{position:'absolute', bottom:'16px', left:'50%', transform:'translateX(-50%)', background:'var(--bg-surface)', border:'1px solid var(--border-light)', borderRadius:'10px', padding:'10px 16px', display:'flex', alignItems:'center', gap:'12px', boxShadow:'0 8px 24px rgba(0,0,0,0.3)', zIndex:20}}>
                      ...
                      <button className="btn-outline" style={{fontSize:'12px', padding:'6px 12px'}}
                        onClick={()=>{
                          ...
                          if (ids.every(id=>selectedIds.has(id))) clearSelection();
                          else setSelectedIds(new Set(ids));
                        }}>
                        {filteredActiveLeads.every(l=>selectedIds.has(String(l.id))) ? 'Barchasini bekor qilish' : 'Hammasini tanlash'}
                      </button>
                  ...
                  {/* Drag paytida o'ng-pastki burchakda paydo bo'ladigan 2ta drop zone */}
                  {draggingLeadId && role !== 'WATCHER' && (() => {
                    ...
                    const zoneStyle = (color, bg) => ({
                      ...
                      fontSize:'13px', fontWeight:700, color, gap:'4px',
                      boxShadow:'0 8px 24px rgba(0,0,0,0.3)', cursor:'copy',
                      transition:'transform 0.15s',
                    });
                    return (
                      <div style={{position:'fixed', right:'24px', bottom:'24px', display:'flex', flexDirection:'column', gap:'10px', zIndex:9999, pointerEvents:'auto'}}>
                        {wonCol && (
                          <div
                            onDragOver={e=>{e.preventDefault(); e.currentTarget.style.transform='scale(1.08)';}}
                            onDragLeave={e=>{e.currentTarget.style.transform='scale(1)';}}
                            onDrop={e=>{ e.currentTarget.style.transform='scale(1)'; handleDrop(e, wonCol.id); }}
                        ...
                        {lostCol && (
                          <div
                            onDragOver={e=>{e.preventDefault(); e.currentTarget.style.transform='scale(1.08)';}}
                            onDragLeave={e=>{e.currentTarget.style.transform='scale(1)';}}
                            onDrop={e=>{ e.currentTarget.style.transform='scale(1)'; handleDrop(e, lostCol.id); }}
                            style={zoneStyle('#ef4444','rgba(239,68,68,0.12)')}>
                            <span style={{fontSize:'26px'}}>❌</span>
                            <span>Muvoffaqiyatsiz</span>
                          </div>
                        )}
                      </div>
                    );
