          ...
          {activeModal && typeof activeModal === 'object' && (() => {
            ...
            const SheetsFlow = () => {
              ...
              const script = [
                ...
                `const UZ_PATTERNS = [`,
                `  { rx: /(ism|name)/i,                     field: 'name_uz' },`,
                `  { rx: /(telefon|raqam|\\btel\\b|phone)/i, field: 'phone_uz' },`,
                `  { rx: /(manzil|shahar|hudud|viloyat|address|city)/i, field: 'region_uz' },`,
                `  { rx: /(email|mail|pochta)/i,            field: 'email_uz' },`,
                `  { rx: /(izoh|sharh|comment|message|xabar)/i, field: 'note_uz' },`,
                `];`,
                ``,
                `// Qiymat haqiqiy telefon raqamga o'xshashligini tekshirish`,
                `function looksLikePhone(s) {`,
                `  if (!s) return false;`,
                `  var digits = s.toString().replace(/\\D/g, '');`,
                ...
                `    var fullName = nmUz || [nmMt, lastN].filter(Boolean).join(' ').trim();`,
                ``,
                ...
                `    }`,
                `    // Agar hech qaysisi to'g'ri ko'rinmasa, birinchi mavjudini olamiz (eski xulq)`,
                `    if (!rawPhone && phoneCandidates.length) rawPhone = phoneCandidates.find(function(x){return !!x;}) || '';`,
                ``,
