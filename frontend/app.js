...
const PipelineEditor = ({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      title: leadCnt + ' ta lead bu bosqichda',
      style: {
        minWidth: '48px',
        textAlign: 'center',
        fontSize: '11px',
        ...
        padding: '5px 8px',
        borderRadius: '6px',
        border: '1px solid ' + (leadCnt > 0 ? 'rgba(99,102,241,0.3)' : 'var(--border-light)')
      }
    }, leadCnt, " \uD83D\uDC64"), isHiddenInKanban && /*#__PURE__*/React.createElement("span", {
      title: "Bu bosqich Kanban ko'rinishidan yashirin (is_won/is_lost)",
      style: {
        fontSize: '10px',
        fontWeight: 700,
        color: '#f59e0b',
    ...
    }, "\uD83D\uDE48 Yashirin"), /*#__PURE__*/React.createElement("button", {
...
const IntegrationsModule = ({
  ...
  const ALL_INTG = [{
    ...
    wh: {
      ...
      url: `${origin}/api/webhook/moizvonki${(() => {
        try {
          const s = JSON.parse(localStorage.getItem('mizon_session') || 'null');
          return s?.companyId ? '?company_id=' + s.companyId : '';
        } catch {
          return '';
        }
      })()}`
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    const SheetsFlow = () => {
      ...
      const script = [`// ╔════════════════════════════════════════════════════╗`, `// ║   Mizon CRM — Google Sheets (Meta Lead Ads)        ║`, `// ║   Extensions → Apps Script ga joylashtiring        ║`, `// ╚════════════════════════════════════════════════════╝`, ``, `const CRM_WEBHOOK  = '${webhookUrl}';`, `const COMPANY_SLUG = '${slug}';`, ``, `// Meta sheet sarlavhalaridan CRM maydonlariga avtomatik moslash`, `// Chap: sheetdagi ustun nomi | O'ng: CRM maydoni`, `// Aniq Meta nomlari`, `const HEADER_MAP = {`, `  'full_name':    'name_meta',`, `  'first_name':   'name_meta',`, `  'last_name':    'name_last',`, `  'phone_number': 'phone_meta',`, `  'phone':        'phone_meta',`, `  'email':        'email_meta',`, `  'city':         'region_meta',`, `  'country':      'region_meta',`, `  'comments':     'note_meta',`, `  'message':      'note_meta',`, `};`, ``, `// MUTLAQO E'TIBORSIZ ustunlar (ID, vaqt, reklama metadata)`, `// Bu ustunlar UZ_PATTERNS ham, HEADER_MAP ham tegmaydi`, `const SKIP_HEADERS = [`, `  'id','lead_id','leadgen_id','created_time','created_date','updated_time',`, `  'ad_id','ad_name','adset_id','adset_name','campaign_id','campaign_name',`, `  'form_id','form_name','page_id','page_name','platform','is_organic',`, `  'partner_name','partner','source','status'`, `];`, ``, `// Foydalanuvchi qo'lda yozgan custom maydonlar (Meta nomlardan USTUN qo'yiladi)`, `// Masalan: "telefon_raqamingizni_yozing!", "ismingizni_yozing!" va h.k.`, `const UZ_PATTERNS = [`, `  { rx: /(\\bism\\b|ismi|ismingiz|familiya|f_i_o|fish)/i, field: 'name_uz' },`, `  { rx: /(telefon|raqam|\\btel\\b|nomer|\\bphone\\b)/i,    field: 'phone_uz' },`, `  { rx: /(manzil|shahar|hudud|viloyat|address)/i,         field: 'region_uz' },`, `  { rx: /(email|mail|pochta)/i,                            field: 'email_uz' },`, `  { rx: /(izoh|sharh|\\bcomment\\b|xabar|\\bnote\\b)/i,    field: 'note_uz' },`, `];`, ``, `// Qiymat haqiqiy telefon raqamga o'xshashligini tekshirish`, `function looksLikePhone(s) {`, `  if (!s) return false;`, `  var str = s.toString();`, `  // ISO timestamp emasligi`, `  if (/^\\d{4}-\\d{2}-\\d{2}[T ]/.test(str)) return false;`, `  var digits = str.replace(/\\D/g, '');`, `  return digits.length >= 7 && digits.length <= 15;`, `}`, `function cleanPhone(s) {`, `  return s.toString().trim().replace(/^p\\s*:/i, '').trim();`, `}`, `// Qiymat haqiqiy ismga o'xshashligini tekshirish`, `function looksLikeName(s) {`, `  if (!s) return false;`, `  var str = s.toString().trim();`, `  if (!str) return false;`, `  if (/^[a-z]+\\s*:\\s*\\d{6,}$/i.test(str)) return false; // Meta id (l:12345...)`, `  if (/^\\d{4}-\\d{2}-\\d{2}[T ]/.test(str)) return false; // timestamp`, `  if (/^\\d{8,}$/.test(str)) return false;                 // sof raqam`, `  return true;`, `}`, ``, `// ─── Asosiy funksiya ──────────────────────────────────────`, `function syncNewLeads() {`, `  const sheet   = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();`, `  const lastRow = sheet.getLastRow();`, `  const props   = PropertiesService.getScriptProperties();`, `  const lastProcessed = parseInt(props.getProperty('mizon_last_row') || '1');`, `  if (lastRow <= lastProcessed) return;`, ``, `  // Sarlavha qatorini o'qib ustun indekslarini aniqlash`, `  const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];`, `  const colIdx  = {};`, `  headers.forEach(function(h, i) {`, `    var hl = (h || '').toString().toLowerCase().trim();`, `    if (!hl) return;`, `    // 0) Noise ustunlarni tashlab yuborish (id, created_time, ad_name, ...)`, `    if (SKIP_HEADERS.indexOf(hl) >= 0) return;`, `    // 1) Aniq Meta nomlari`, `    var exact = HEADER_MAP[hl];`, `    if (exact && colIdx[exact] === undefined) colIdx[exact] = i;`, `    // 2) Custom (Uzbek/foydalanuvchi yozgan) maydonlar — bular ustunroq`, `    UZ_PATTERNS.forEach(function(p) {`, `      if (HEADER_MAP[hl]) return; // aniq Meta nomi bo'lsa custom emas`, `      if (p.rx.test(hl) && colIdx[p.field] === undefined) colIdx[p.field] = i;`, `    });`, `  });`, `  Logger.log('Aniqlangan ustunlar: ' + JSON.stringify(colIdx));`, ``, `  const startRow = lastProcessed + 1;`, `  const data = sheet.getRange(startRow, 1, lastRow - lastProcessed, sheet.getLastColumn()).getValues();`, ``, `  var processed = lastProcessed;`, `  for (var i = 0; i < data.length; i++) {`, `    var row = data[i];`, ``, `    // ISM: avval foydalanuvchi yozgan custom maydon, keyin Meta full_name`, `    var nmUz  = colIdx['name_uz']   !== undefined ? String(row[colIdx['name_uz']]   || '').trim() : '';`, `    var nmMt  = colIdx['name_meta'] !== undefined ? String(row[colIdx['name_meta']] || '').trim() : '';`, `    var lastN = colIdx['name_last'] !== undefined ? String(row[colIdx['name_last']] || '').trim() : '';`, `    var fullName = '';`, `    if (looksLikeName(nmUz)) fullName = nmUz;`, `    else if (looksLikeName(nmMt) || looksLikeName(lastN)) fullName = [nmMt, lastN].filter(Boolean).join(' ').trim();`, ``, `    // TELEFON: bir nechta nomzodlar — birinchisi haqiqiy raqamga o'xshasa olamiz`, `    var phoneCandidates = [];`, `    if (colIdx['phone_uz']   !== undefined) phoneCandidates.push(cleanPhone(row[colIdx['phone_uz']]   || ''));`, `    if (colIdx['phone_meta'] !== undefined) phoneCandidates.push(cleanPhone(row[colIdx['phone_meta']] || ''));`, `    var rawPhone = '';`, `    for (var j = 0; j < phoneCandidates.length; j++) {`, `      if (looksLikePhone(phoneCandidates[j])) { rawPhone = phoneCandidates[j]; break; }`, `    }`, `    // Fallback: hamma ustunlardan haqiqiy telefon qidirish`, `    if (!rawPhone) {`, `      for (var k = 0; k < row.length; k++) {`, `        if (k === colIdx['phone_uz'] || k === colIdx['phone_meta']) continue;`, `        var cand = cleanPhone(row[k] || '');`, `        if (looksLikePhone(cand)) { rawPhone = cand; break; }`, `      }`, `    }`, ``, `    var email  = (colIdx['email_uz']  !== undefined ? row[colIdx['email_uz']]  : (colIdx['email_meta']  !== undefined ? row[colIdx['email_meta']]  : '')) || '';`, `    var region = (colIdx['region_uz'] !== undefined ? row[colIdx['region_uz']] : (colIdx['region_meta'] !== undefined ? row[colIdx['region_meta']] : '')) || '';`, `    var note   = (colIdx['note_uz']   !== undefined ? row[colIdx['note_uz']]   : (colIdx['note_meta']   !== undefined ? row[colIdx['note_meta']]   : '')) || '';`, ``, `    var payload = {`, `      company_slug: COMPANY_SLUG,`, `      name:   fullName,`, `      phone:  rawPhone,`, `      email:  String(email).trim(),`, `      region: String(region).trim(),`, `      note:   String(note).trim(),`, `      row_index: startRow + i,`, `    };`, `    if (!payload.name && !payload.phone) { processed++; continue; }`, `    try {`, `      var resp   = UrlFetchApp.fetch(CRM_WEBHOOK, {`, `        method: 'post', contentType: 'application/json',`, `        payload: JSON.stringify(payload), muteHttpExceptions: true,`, `      });`, `      var result = JSON.parse(resp.getContentText());`, `      Logger.log('Qator '+(startRow+i)+': '+(result.duplicate?'⚠️ Takrorlangan':'✅ id='+result.id));`, `    } catch(e) { Logger.log('Xato: '+e.message); }`, `    processed++;`, `    props.setProperty('mizon_last_row', String(processed));`, `  }`, `}`, ``, `// ─── Trigger o'rnatish — BIR MARTA ishga tushiring ───────`, `function createTrigger() {`, `  ScriptApp.getProjectTriggers()`, `    .filter(function(t){ return t.getHandlerFunction() === 'syncNewLeads'; })`, `    .forEach(function(t){ ScriptApp.deleteTrigger(t); });`, `  ScriptApp.newTrigger('syncNewLeads').timeBased().everyMinutes(5).create();`, `  Logger.log("✅ Trigger o'rnatildi — har 5 daqiqada sinxronlanadi");`, `}`].join('\n');
      const saveGs = async () => {
        setGsSaving(true);
        try {
    ...
    const CapiFlow = () => {
      ...
      const saveCapi = async () => {
        ...
        setSaving(true);
        try {
          const r = await fetch('/api/integrations/meta-capi', {
            method: 'PUT',
            headers: authH(),
            body: JSON.stringify({
          ...
          setAccessToken('');
          setShowToken(false);
          const r2 = await fetch('/api/integrations/meta-capi', {
            headers: authH()
          });
      ...
      const testCapi = async () => {
        setTStat('loading');
        try {
          const r = await fetch('/api/integrations/meta-capi/test', {
            method: 'POST',
            headers: authH(),
            body: JSON.stringify({})
          });
          ...
          if (r.ok && d.success) {
            setTStat({
              ok: true,
              msg: `✅ Meta qabul qildi (${d.events_received} event)`,
              details: d
            });
          } else {
            setTStat({
      ...
      const disconnectCapi = async () => {
        ...
        try {
          await fetch('/api/integrations/meta-capi', {
            method: 'DELETE',
            headers: authH()
          });
          const upd = {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        n: 1,
        title: 'Events Manager\'ni oching',
        desc: 'business.facebook.com/events_manager2 → Pixel/Dataset tanlang yoki yarating'
      }, {
        n: 2,
        title: 'Conversions API Access Token oling',
        desc: 'Pixel → Settings → "Conversions API" bo\'limi → "Generate Access Token" — App Review kerak EMAS'
      }, {
        n: 3,
        title: 'Pixel ID + Token\'ni quyiga kiriting va saqlang',
        desc: 'CRM avtomatik ravishda Lead/Purchase eventlarini Meta\'ga server-tomondan yuboradi'
      }].map(s => /*#__PURE__*/React.createElement("div", {
        key: s.n,
        style: {
          display: 'flex',
      ...
      }, s.desc))))), cfg?.configured && /*#__PURE__*/React.createElement("div", {
        style: {
          background: 'rgba(1,167,80,0.06)',
      ...
      }, "\u25CF Ulangan"), /*#__PURE__*/React.createElement("div", {
        style: {
          color: 'var(--text-secondary)'
        }
      }, "Pixel ID: ", /*#__PURE__*/React.createElement("code", {
        style: {
          background: 'var(--surface-variant)',
          padding: '1px 6px',
          borderRadius: '4px'
        }
      }, cfg.pixel_id)), /*#__PURE__*/React.createElement("div", {
        style: {
          color: 'var(--text-muted)',
          fontSize: '11px',
          marginTop: '3px'
        }
      }, "Token: ", cfg.access_token_masked), cfg.test_event_code && /*#__PURE__*/React.createElement("div", {
        style: {
          color: 'var(--text-muted)',
          fontSize: '11px',
          marginTop: '3px'
        }
      }, "Test code: ", /*#__PURE__*/React.createElement("code", null, cfg.test_event_code))), /*#__PURE__*/React.createElement("div", {
        style: {
          marginBottom: '12px'
        }
      }, /*#__PURE__*/React.createElement("span", {
        className: "label-sm"
      }, "Pixel ID (Dataset ID) *"), /*#__PURE__*/React.createElement("input", {
        className: "input-base",
        type: "text",
        ...
        className: "label-sm"
      }, "Conversions API Access Token *"), /*#__PURE__*/React.createElement("div", {
        style: {
          position: 'relative'
        }
      }, /*#__PURE__*/React.createElement("input", {
      ...
      }, showToken ? '🙈' : '👁')), cfg?.configured && !accessToken && /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: '10px',
          color: 'var(--text-muted)',
          marginTop: '4px'
        }
      }, "Token allaqachon saqlangan. O'zgartirish uchun yangisini kiriting.")), /*#__PURE__*/React.createElement("div", {
        style: {
          marginBottom: '12px'
        }
      }, /*#__PURE__*/React.createElement("span", {
        className: "label-sm"
      }, "Test Event Code (ixtiyoriy \u2014 diagnostika uchun)"), /*#__PURE__*/React.createElement("input", {
        className: "input-base",
        type: "text",
      ...
      }, "Events Manager \u2192 \"Test Events\" tabidan oling. Faqat sinov uchun!")), tStat && tStat !== 'loading' && /*#__PURE__*/React.createElement("div", {
        style: {
          background: tStat.ok ? 'rgba(1,167,80,0.08)' : 'rgba(239,68,68,0.08)',
          border: `1px solid ${tStat.ok ? 'rgba(1,167,80,0.3)' : 'rgba(239,68,68,0.3)'}`,
          borderRadius: '8px',
          padding: '10px 12px',
          marginBottom: '12px',
          fontSize: '12px'
        ...
        style: {
          fontWeight: 700,
          color: tStat.ok ? '#01a750' : '#ef4444'
        }
      }, tStat.msg), tStat.details?.fbtrace_id && /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: '10px',
          color: 'var(--text-muted)',
          marginTop: '4px',
      ...
      }, "fbtrace_id: ", tStat.details.fbtrace_id), tStat.details?.messages && tStat.details.messages.length > 0 && /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: '10px',
          color: 'var(--text-muted)',
          marginTop: '4px'
        }
      }, tStat.details.messages.join(' • '))), /*#__PURE__*/React.createElement("div", {
        style: {
          display: 'flex',
          gap: '8px',
          flexWrap: 'wrap',
        ...
        disabled: saving,
        onClick: saveCapi
      }, saving ? '⏳ Saqlanmoqda...' : cfg?.configured ? '💾 Yangilash' : '✅ Saqlash'), cfg?.configured && /*#__PURE__*/React.createElement("button", {
        onClick: testCapi,
        disabled: tStat === 'loading',
        style: {
          padding: '10px 14px',
          background: 'var(--surface-variant)',
          border: '1px solid var(--outline-variant)',
      ...
      }, tStat === 'loading' ? '⏳ Yuborilmoqda...' : '🧪 Test event'), cfg?.configured && /*#__PURE__*/React.createElement("button", {
        style: {
          padding: '10px 12px',
          background: 'none',
          border: '1px solid rgba(239,68,68,0.35)',
      ...
      }, "\u2139\uFE0F Qachon eventlar yuboriladi?"), "Hozircha qo'lda Test orqali. Avtomatik triggerlar (lead WON \u2192 Purchase, lead yaratilganda \u2192 Lead) keyingi versiyada qo'shiladi. Yuboriluvchi PII (email/telefon) Meta talabiga ko'ra SHA-256 bilan hash qilinadi."));
    };
    ...
    }, "\u2715")), activeModal.key === 'facebook' && /*#__PURE__*/React.createElement(FbOAuthFlow, null), activeModal.key === 'instagram' && /*#__PURE__*/React.createElement(IgOAuthFlow, null), activeModal.key === 'google_sheets' && /*#__PURE__*/React.createElement(SheetsFlow, null), activeModal.key === 'meta_capi' && /*#__PURE__*/React.createElement(CapiFlow, null), !isCustomUI && /*#__PURE__*/React.createElement("div", {
      style: {
        padding: '20px 22px'
      }
...
const ActivityLogPanel = ({
  ...
  const renderLogRow = log => {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, tabDef.icon)), /*#__PURE__*/React.createElement("div", {
      style: {
        flex: 1,
        minWidth: 0
      }
    }, isStage && log.fromStatus && log.toStatus ? /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: '12px',
        color: 'var(--text-secondary)',
        display: 'flex',
    ...
    }, fromT), /*#__PURE__*/React.createElement("span", {
      className: "material-symbols-outlined",
      style: {
        fontSize: '14px',
        color: '#6366f1'
    ...
    }, toT), log._legacy && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: '9px',
        color: 'var(--text-muted)',
        fontStyle: 'italic'
      ...
      style: {
        fontSize: '10px',
        color: knownUsernames.has(log.by) ? 'var(--text-muted)' : '#f59e0b'
      }
    }, "by ", log.by, !knownUsernames.has(log.by) && ' ⚠️'), log.assignee && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: '10px',
        color: '#f59e0b'
      }
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "(jami ", all.length, " ta yozuv)"))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: '6px',
      padding: '10px 20px',
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      style: {
        ...
        background: isActive ? `${t.color}18` : 'var(--bg-base)',
      ...
      style: {
        background: isActive ? t.color : 'var(--surface-variant)',
  ...
  }, "person"), /*#__PURE__*/React.createElement("select", {
    className: "input-base",
    style: {
      marginBottom: 0,
      height: '36px',
    ...
    value: "all"
  }, "Barcha xodimlar"), allBys.map(b => /*#__PURE__*/React.createElement("option", {
    key: b,
    value: b
  }, b, !knownUsernames.has(b) && ' ⚠️')))), tab === 'stage' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: '6px'
  ...
  }, "Bosqichdan:"), /*#__PURE__*/React.createElement("select", {
    className: "input-base",
    style: {
      marginBottom: 0,
      height: '36px',
    ...
    value: "all"
  }, "\u2014 Barchasi \u2014"), stageOptions.map(s => /*#__PURE__*/React.createElement("option", {
    key: s.id,
    value: String(s.id)
  ...
  }, "Bosqichga:"), /*#__PURE__*/React.createElement("select", {
    className: "input-base",
    style: {
      marginBottom: 0,
      height: '36px',
  ...
  }, totalCount, " ta ", tab === 'stage' ? "o'tish" : "yozuv")), tab === 'stage' && transitionStats.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '12px 20px',
      borderBottom: '1px solid var(--outline-variant)',
  ...
  }, "\uD83D\uDCCA Eng ko'p uchragan o'tishlar ", empFilter !== 'all' && `(${empFilter})`), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexWrap: 'wrap',
      gap: '8px'
    }
  }, transitionStats.map(([trans, cnt]) => /*#__PURE__*/React.createElement("div", {
    key: trans,
    style: {
      display: 'flex',
      alignItems: 'center',
  ...
  }, cnt))))), shown.length === 0 ? /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '30px',
      textAlign: 'center',
      color: 'var(--text-muted)',
  ...
  }, "Tanlangan filtrga mos yozuv yo'q") : /*#__PURE__*/React.createElement("div", {
    style: {
      maxHeight: '500px',
      overflowY: 'auto'
    }
  }, shown.map(renderLogRow), !showAll && totalCount > 200 && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '12px',
      textAlign: 'center',
      background: 'var(--bg-base)'
  ...
  }, "Yana ", totalCount - 200, " ta yozuvni ko'rsatish")), showAll && totalCount > 200 && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: '12px',
      textAlign: 'center',
      background: 'var(--bg-base)'
...
const HisobotlarModule = ({
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    if (sortedOwners.length === 0) return /*#__PURE__*/React.createElement("div", {
      style: {
        color: 'var(--text-muted)',
        fontSize: '13px',
        textAlign: 'center',
    ...
    return sortedOwners.map(op => {
      ...
      if (isPlaceholder) {
        ...
        if (lower === 'navbatda') displayName = '⏳ Tayinlanmagan (queue)';else if (lower === 'ceo') displayName = `⚠️ Tizim default ('ceo')`;else displayName = `⚠️ Noma'lum xodim: ${op}`;
      }
      return /*#__PURE__*/React.createElement("div", {
        key: op,
        style: {
      ...
      }, isPlaceholder ? '⚠️' : op[0].toUpperCase()), /*#__PURE__*/React.createElement("div", {
        style: {
          flex: 1
        }
      }, /*#__PURE__*/React.createElement("div", {
        ...
        title: isPlaceholder ? `Ushbu lead'lar mas'uli "${op}" — kompaniya xodimlar ro'yxatida yo'q. Lid kartasidan to'g'ri mas'ulni belgilang.` : ''
  ...
  })))), /*#__PURE__*/React.createElement(ActivityLogPanel, {
    leads: leads,
...
const App = () => {
          ...
          incoming.forEach(l => {
            if (!prev.has(l.id)) {
              ...
              if (src !== 'manual') {
                ...
                addNotifRef.current('new_lead', `${icon} Yangi lid keldi`, `${l.name || "Noma'lum"} — ${src.replace('meta_', '').replace('_', ' ')}${l.phone ? ' • ' + l.phone : ''}`, l.id);
              }
            }
          });
        }
  ...
  const exportLeadsToCsv = customRows => {
    ...
    rows.forEach(l => {
      lines.push([l.id, l.name, l.phone, l.region, l.source, stageTitle(l.status), l.owner || '', l.taskDescription || '', l.deadline ? new Date(l.deadline).toLocaleString('uz-UZ') : '', l.createdAt ? new Date(l.createdAt).toLocaleString('uz-UZ') : '', l.actualCallAttempts || 0].map(esc).join(','));
    });
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    n: "download",
    s: 13
  }), " Eksport (CSV)"), role !== 'WATCHER' && /*#__PURE__*/React.createElement("button", {
    className: "btn-primary",
